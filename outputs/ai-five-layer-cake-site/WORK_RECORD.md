# AI 双维矩阵产业链知识体系网站 - 工作记录

## 当前状态快照（2026-06-24）

项目当前定位为“AI 产业链学习笔记 + 科普展示页 + 股票研究资料库”。首页已收口为三段式入口，公司研究页已完成第二轮体验优化，搜索和来源页已收口，深度研究内容直接在公司详情中呈现。

当前优先级：

- P3.4：重点赛道深挖已启动，已完成 `HBM / AI 存储瓶颈`、`光互联 / OCS / AI 网络硬件` 与 `AI 电力 / 数据中心能源` 三篇专题。
- P3.5：来源质量分级第一轮完成，来源页、来源徽章、公司卡和专题公司卡统一显示来源质量层级。
- P3.6 / P3.7：当前不推进，暂不做公司更新时间、催化剂日期、研究优先级或新增导出型简报功能。
- Codex 工作流收口：入口文档改为 `README.md` + 本工作记录，常用检查脚本统一为 `scripts/check_site.sh`。
...
本文件是当前主工作记录。`OpenClaw_workspace/AI/Research/ai_five_layer_cake_site_work_record.md` 仅作为历史归档和旧上下文参考，不作为每次启动的必读入口。

## 2026-07-01 更新：P3.5 来源质量分级第一轮

用户要求开始 P3.5，且 P3.6、P3.7 不需要做。本轮只做来源质量分级，不新增公司研究更新时间、催化剂日期、研究优先级，也不新增导出型简报能力。

本轮调整：
- 新增 `source-quality.js`，统一来源质量判断入口。
- 来源质量分为：官方/监管、财报/年报、公司资料、技术资料、机构/框架、研究参考、待核验。
- 来源页复用统一质量函数，支持按 `质量：...` 筛选，并在来源卡顶部直接显示质量层级。
- `source-badges.js` 复用统一质量函数，专题页和深度页来源徽章显示为“发布方 · 来源质量”。
- 首页公司卡、专题公司卡的来源质量摘要复用统一质量函数，避免各处判断口径不一致。
- 所有使用来源徽章的页面已接入 `source-quality.js`。

验证：
- `scripts/check_site.sh` 通过。
- 18 个 HTML 文件本地引用检查通过，缺失为 0。
- 未推进 P3.6 / P3.7。

## 2026-07-01 更新：P3.4-3 AI 电力 / 数据中心能源专题

用户要求开始 AI 电力专题，并要求将“重点赛道深挖”入口从总览区移到产业链页最下方，同时去掉 `P3 Focus`、`P3.4-x` 等项目管理式小标题。

本轮调整：
- 新增 `pages/ai-power.html`，作为第三篇重点赛道深挖专题。
- 专题结构继续按“这是什么 / 为什么重要 / 产业链位置 / 相关公司 / 看什么指标 / 风险 / 来源”组织。
- 覆盖 PUE、WUE、PPA、并网、电网设备、燃气轮机、核能、储能、液冷和数据中心选址。
- 相关公司覆盖 Vertiv、Schneider Electric、Eaton、Constellation、NextEra、GE Vernova、Siemens Energy、Delta Electronics、英维克、科华数据、申菱环境、Digital Realty、Equinix、GDS、CoreWeave、Microsoft、Amazon、Alphabet 等。
- 首页总览区不再展示重点赛道深挖入口；入口统一移动到产业链页底部，并新增页内跳转按钮。
- 重点赛道入口卡片改为面向读者的分类标签：存储瓶颈、网络硬件、能源瓶颈，不再显示项目编号。
- 全局搜索新增 `AI 电力 / 数据中心能源` 专题项，支持搜索 AI 电力、PUE、WUE、PPA、并网、电网设备、燃气轮机、核能、储能、液冷等关键词。
- 专题底部新增“可复制学习笔记 / 股票研究简报”结构。

内容边界：
- 本页只作为学习和股票研究框架，不构成投资建议。
- 电力专题不写成“所有电力链公司都受益”，而是拆成电力容量、并网周期、PUE/WUE、PPA 合同、液冷导入、数据中心预租率和 CAPEX 回报等变量。

验证：
- `scripts/check_site.sh` 通过。
- 18 个 HTML 文件本地引用检查通过，缺失为 0。

## 2026-06-30 更新：P3.4-1 HBM / AI 存储瓶颈专题

用户确认 P3 后续进入重点赛道深挖，并要求在存储专题中加入“AI 产能挤出效应”：当海力士、美光、三星、长鑫等把产能和资本开支投向 AI 数据中心存储建设时，原本服务消费电子、汽车电子、工业设备等低毛利市场的存储供给由谁补位。

本轮调整：
- 新增 `pages/hbm-storage.html`，作为 P3.4 第一篇重点赛道深挖专题。
- 专题结构按“这是什么 / 为什么重要 / 产业链位置 / 相关公司 / 看什么指标 / 风险 / 来源”组织。
- 新增 `AI 挤出效应` 章节，分层讨论：
  - 普通 DRAM：长鑫、南亚科、华邦电等可能承接部分 DDR4、LPDDR、利基 DRAM 缺口。
  - 车规 / 工业 / 小容量存储：北京君正、兆易创新、普冉、东芯、聚辰、ISSI、Winbond 更适合跟踪认证、生命周期和可靠性。
  - NAND / SSD / 模组：长江存储、江波龙、佰维、德明利、群联、慧荣、香农芯创对应颗粒、模组、控制器或分销链条。
  - 核心判断不写成“低端存储全都受益”，而是拆成供给缺口、客户认证、库存周期、涨价能力和国产替代五个变量。
- 首页新增重点赛道深挖入口，当前开放 HBM / AI 存储瓶颈，光互联 / OCS 和 AI 电力作为后续专题提示。
- 芯片层页和 AI 芯片供应链页增加 HBM 专题入口。
- 全局搜索新增“专题”类型，支持搜索 HBM、DRAM、NAND、SSD、AI 挤出效应、存储等关键词命中新专题。
- 来源页新增按“来源质量”筛选，由现有 type/confidence 派生：一手来源、高质量参考、公司资料、研究参考、待核验。
- 公司详情中新增来源质量摘要，先基于已有 sourceIds 派生，不批量手填 224 家公司更新时间。
- HBM 专题页底部新增“可复制学习笔记 / 股票研究简报”结构。

来源处理：
- Longbridge / 海豚研究截图仅作为研究线索，不作为唯一来源。
- 网站内核心事实优先绑定 Micron、Samsung、SK hynix、CXMT、YMTC、GigaDevice、ISSI、Winbond 等官方或公司来源入口；后续写更细事实时仍需继续补财报、电话会、10-Q/年报或 IR 材料。

验证：
- `scripts/check_site.sh` 通过。
- 16 个 HTML 文件本地引用检查通过，缺失为 0。

## 2026-07-01 更新：P3.4-2 光互联 / OCS / AI 网络硬件专题

用户要求继续下一步。本轮执行 P3.4-2，新增第二篇重点赛道深挖专题，聚焦 AI 集群从单卡竞争进入网络 fabric 竞争后的光互联、OCS 和网络硬件价值链。

本轮调整：
- 新增 `pages/optical-ocs.html`，作为 P3.4 第二篇重点赛道深挖专题。
- 专题结构继续按“这是什么 / 为什么重要 / 产业链位置 / 相关公司 / 看什么指标 / 风险 / 来源”组织。
- 首页重点赛道深挖入口将 `光互联 / OCS` 从 pending 改成可点击专题。
- 基础设施层页和数据中心链页增加光互联 / OCS 专题入口。
- 全局搜索新增“光互联 / OCS / AI 网络硬件”专题项，支持搜索 OCS、CPO、LPO、AEC、SerDes、PCB、CCL、AI 网络等关键词。
- Source Registry 新增 `google-apollo-ocs`，用于研究 OCS 在大规模数据中心网络中的部署经验和工程边界。
- 专题内将 OCS 写成长期可重构网络路线，不写成所有 AI 集群都会立即采用的确定商用路径。
- 专题底部新增“可复制学习笔记 / 股票研究简报”结构。

内容边界：
- 当前主线仍是 800G/1.6T 光模块、交换芯片、SerDes/DSP、AEC、PCB/CCL 和连接器。
- OCS/CPO/LPO 作为技术路线和架构变量跟踪，必须继续用客户部署、功耗、成本、良率、调度复杂度和订单证据验证。
- 公司池承接现有光互联与高速 PCB 同业对照，不批量修改公司数据结构。

验证：
- `scripts/check_site.sh` 通过。
- 17 个 HTML 文件本地引用检查通过，缺失为 0。

## 当前任务背景

用户希望构建一个可用于投资研究与创业战略分析的【AI 双维矩阵产业链知识体系网站】。
...
- 先做一版网站初稿，先包含整个五层蛋糕框架。
- 后续再一层一层填补内容。
- 保存工作记录，方便上下文用完后在新对话里继续。
...
1. v0.1 静态骨架：先用 HTML/CSS/JS 做双维矩阵、五层框架、八条产业链、前沿雷达。
2. v0.2 逐层填充：每次只研究一层，补基础知识、2026 前沿、黑话词典、公司图谱和来源。
3. v0.3 知识工程化：迁移到 Astro + MDX + React Islands，让内容与代码分离。
4. v1.0 交互知识库：加入 React Flow/D3、路径高亮、术语悬浮、全文搜索、来源管理和版本记录。
...
```text
/Users/michaeldong/Code/New project/outputs/ai-five-layer-cake-site/
```
...
- 两条主线：纵向传导 / 横向展开
- 双维矩阵：
  - 5 行：能源、芯片、基础设施、模型、应用
  - 8 列：训练、推理、企业落地、数据中心、芯片供应链、机器人、Agent、安全评测
  - 共 40 个交点，占位描述已填
- 五层主框架卡片：
  - 核心矛盾
  - 基础支点
...
当前内容是“结构与初始判断”，不是最终研究稿。

正式逐层填充时必须联网核验并记录来源，尤其是：

- NVIDIA Rubin / Vera Rubin / NVLink 6 / GB200 NVL72
- HBM4、CoWoS-L/R、先进封装产能
- SMR、小型核能、PPA、AI 数据中心电力交易
- o1/o3 类 Reasoning Model、Prefill/Decode、KV Cache、Speculative Decoding
- Agent 原生工作流、MCP、企业权限、安全评测
- 具身智能、VLA、Sim-to-Real、端侧低延迟推理
...
```bash
python3 -m http.server 8989 --directory outputs/ai-five-layer-cake-site
```

注意：本轮尝试启动本地预览服务时，因为当前工具权限审批超时，未完成浏览器视觉验证。已通过 Node 方式做过静态结构检查，结果显示核心容器和数据数量存在。

## 2026-05-21 更新：能源层已开始填充

用户要求“先从纵向来看，能源层开始”。

已完成：

- 在网站新增 `能源层：Power` 深度看板。
- 已写入模块：
  - 核心判断
  - 已核验事实
  - 经典基础知识筑基
...
- Amazon / X-energy - Small modular reactor and nuclear energy partnerships
- Constellation - Crane Clean Energy Center / Microsoft PPA

已写入网站的关键事实：

- IEA 2026 报告：2025 年全球数据中心用电增长 17%，AI 专用数据中心用电增长 33%，数据中心约占全球电力需求 1.5%。
- Google 与 Kairos Power 签署协议，计划到 2035 年部署 500MW 先进核能。
- Amazon 投资 X-energy，相关合作目标是在美国部署超过 5GW 新核能项目。
- Microsoft 与 Constellation 签署 20 年购电协议，推动 Three Mile Island 1 号机组重启，为数据中心供电。

注意事项：

- 能源层已经具备第一版研究看板，但未来可以继续扩写：
  - 美国电网并网队列数据
  - 变压器 / 开关柜 / UPS / CDU 供应链公司
  - PPA 定价、核能项目融资、燃气轮机供需
  - 中国、美国、欧洲、中东的数据中心电力差异

## 下一轮建议

最推荐下一步：继续完善【能源层 Power】的横向矩阵交点，或者进入【基础设施层 Infra】。

原因：

- 它连接能源、芯片、模型、应用，是整个蛋糕的中间枢纽。
- 可以自然展开 AI Factory、100K 卡集群、液冷、高速网络、推理云、GPU 利用率。
- 研究完 Infra 后，芯片层和能源层的需求会更清楚。

下一轮可直接对 Codex 说：

```text
继续 AI 双维矩阵产业链知识体系网站项目。
请先读取 /Users/michaeldong/Code/New project/outputs/ai-five-layer-cake-site/WORK_RECORD.md，
然后打开当前网站文件，开始深挖【基础设施层 Infra】：
1. 联网核验 2025-2026 最新资料；
2. 补充基础知识、前沿技术、黑话词典、玩家图谱、瓶颈、商业价值；
3. 把内容填回网站；
4. 更新 WORK_RECORD.md。
```
...
- 用户偏好中文 Markdown、结构化、直接可用的输出。
- 网站应偏“投资研究终端 + 硅谷战略咨询”风格，不要做花哨营销页。
- 后续每个事实型前沿判断都需要来源、日期和置信度。
- 不要把未经核验的热门说法写成确定事实。

## 2026-05-21 更新：知识点按钮已接入内容面板

用户指出：网站里的小 button / 胶囊标签不能只是空壳，点进去必须有实际内容。

已完成：

- 新增统一详情弹窗 `detailModal`。
- 五层卡片里的胶囊标签现在可点击：
  - 关键细分
  - 关键指标
  - 初始前沿方向
...
- 已深挖的能源层内容有更完整知识卡：
  ...
  - 电力-冷却一体化
  - 电力 / 电网 / 储能 / 天然气 / 核能
- 其他未深挖层级也不再是空壳，会打开基础解释 + 所属层级/链条 + 后续填充方向。

校验：

- 静态脚本语法通过。
- 已确认存在详情弹窗、知识卡数据、通用兜底、胶囊按钮、链条按钮、矩阵按钮、雷达按钮和样式。

## 2026-05-21 更新：首页内容边界与层级页

用户要求：

- 每层蛋糕都要有当前行业现状。
- 内容较多的标签或节点，应跳转到新网页，不要都放在首页。
- 网站创建计划和工作记录不要放在网站前台，网站里只保留 AI 五层蛋糕相关内容。

已完成：

- 从首页删除前台展示的可行性评估、网站演进路线、建议填充顺序等内部建站内容。
- 首页导航只保留 AI 五层蛋糕相关入口：总览、矩阵、五层、产业链、层间联系、前沿雷达。
- 五层卡片新增“当前行业现状”：
  - 能源层
  - 芯片层
  - 基础设施层
...
- 新增五个独立层级页：
  ...
  - `pages/app.html`
- 首页每层卡片新增“进入深度页”链接。
- 能源层重内容已迁移到 `pages/power.html`，包括行业现状、基础知识、前沿路线、词典、玩家图谱和来源。
- 能源层重内容标签点击后跳转到能源层独立页对应锚点，例如：
  - PPA -> `pages/power.html#ppa`
  - SMR -> `pages/power.html#smr`
  - Interconnection Queue -> `pages/power.html#interconnection-queue`
  - 电力-冷却一体化 -> `pages/power.html#power-cooling`

校验：

- 首页已不包含“可行性 / 网站演进 / 建站 / 工作记录”等内部项目文字。
- 五个层级页均存在，且均包含行业现状或当前判断。
- 脚本语法校验通过。

## 2026-05-21 更新：芯片层已填充

用户要求进入下一层【芯片层】。

已完成：

- 将 `pages/silicon.html` 从骨架页升级为完整芯片层深度页。
- 已覆盖：
  - 当前行业现状
  - 已核验事实
  - 经典基础知识筑基
  ...
  - 来源链接
- 首页芯片层重标签已跳转到芯片页对应锚点：
  - Rubin/Vera Rubin -> `pages/silicon.html#rubin`
  - HBM4 -> `pages/silicon.html#hbm4`
  - CoWoS-L/R -> `pages/silicon.html#cowos`
  - CPO -> `pages/silicon.html#silicon-photonics`
  - 推理 ASIC 与硅光互联 -> `pages/silicon.html#inference-asic`
...
- 芯片层已经从“谁的 GPU 单卡性能最高”进入“谁能交付 rack-scale AI supercomputer”的系统战。
- HBM4、CoWoS/3DFabric、NVLink 6、硅光/CPO、SerDes/Retimer、电源和液冷共同决定 AI 算力实际供给。
- NVIDIA 的真实护城河不只是 GPU，而是 GPU + CPU + 网络 + DPU + 软件 + 机柜级系统的整个平台。

## 2026-05-21 更新：全站术语小弹窗系统

用户要求：

- 所有内容的重要名词都要做名词解释小弹窗。
- 用术语小弹窗替代“核心名词黑话词典”模块。
...
- 新增全站术语脚本：
  - `glossary.js`
  - `page-glossary.js`
- 新增术语小弹窗样式 `.term` 和 `.term-popup`。
- 首页已加载术语系统。
- 能源层页 `pages/power.html` 已接入术语小弹窗。
- 芯片层页 `pages/silicon.html` 已接入术语小弹窗。
- 删除能源层页和芯片层页里的独立“核心名词黑话词典 / Glossary”模块。
- 当前术语库已有约 40+ 个核心名词解释，包括：
  - PPA、SMR、PUE/WUE、Interconnection Queue、表后供电、CDU、液冷、基荷
  - GPU、HBM、HBM4、CoWoS、CoWoS-L/R、NVLink、NVLink 6、CPO、SerDes、Retimer、EUV、High-NA EUV、ABF 基板等

校验：

- 已确认能源层页和芯片层页不再包含独立 Glossary 区块。
- 能源层页有 17 个正文术语按钮。
- 芯片层页有 53 个正文术语按钮。
- 术语脚本语法通过。

## 2026-05-21 更新：基础设施层已填充

用户要求进入【基础设施层】。

已完成：

- 将 `pages/infra.html` 从骨架页升级为完整基础设施层深度页。
- 按当前规范完成：
  - 当前行业现状
  - 已核验事实
  - 经典基础知识筑基
  ...
  - 正文术语小弹窗
- 不再单独放“核心名词黑话词典”模块。
- 首页基础设施层重标签跳转到基础设施页对应锚点：
  - 100K 卡级 AI Factory -> `pages/infra.html#ai-factory`
  - Scale-up/Scale-out 双网络 -> `pages/infra.html#scale-network`
  - 全液冷高密机柜 -> `pages/infra.html#liquid-rack`
  - 推理云调度 -> `pages/infra.html#disaggregated-inference`
  - MFU/MBU、MTBF、Collective Communication、CDU -> `pages/infra.html#bottlenecks` 或相关锚点
- `glossary.js` 新增基础设施术语：
  - GB200 NVL72
  - Dynamo
  - Prefill/Decode
  - KV Cache
...
- NVIDIA - Dynamo
- Meta Engineering - Training Llama 3 with 24K GPU clusters
- xAI - Colossus
- OpenAI - Stargate Project

关键结论：

- 基础设施层从传统云计算升级为 AI Factory，核心不是“有多少卡”，而是可持续训练吞吐、推理吞吐、故障恢复、液冷、电力、网络和可售 token 产能。

## 2026-05-21 更新：模型层已填充

用户要求进入【模型层】。

已完成：

- 将 `pages/model.html` 从骨架页升级为完整模型层深度页。
- 按当前规范完成：
  - 当前行业现状
  - 已核验事实
  - 经典基础知识筑基
...
- 不再单独放“核心名词黑话词典”模块。
- 首页模型层重标签跳转到模型页对应锚点：
  - o1/o3 类 Reasoning Model -> `pages/model.html#reasoning`
  - Prefill/Decode 分离 -> `pages/model.html#inference-optimization`
  - KV Cache 优化 -> `pages/model.html#inference-optimization`
  - 多模态与世界模型 -> `pages/model.html#world-model`
  - 小模型、蒸馏与端侧模型 -> `pages/model.html#small-model`
- `glossary.js` 新增模型层术语：
  - Reasoning Model
  - Reasoning Token
  - Test-Time Compute
...
- 模型层竞争从 pretrain scaling 转向 inference-time compute、tool use、long context、multimodal grounding 和单位 token 经济性。

## 2026-05-21 更新：应用层已填充

用户要求进入【应用层】。

已完成：

- 将 `pages/app.html` 从骨架页升级为完整应用层深度页。
- 按当前规范完成：
  - 当前行业现状
  - 已核验事实
  - 经典基础知识筑基
...
- 不再单独放“核心名词黑话词典”模块。
- 首页应用层重标签跳转到应用页对应锚点：
  - Agent 原生工作流 -> `pages/app.html#agentic-workflow`
  - AI 编程工作台 -> `pages/app.html#coding-agent`
  - 垂直行业 Autopilot -> `pages/app.html#vertical-ai`
  - 具身智能应用 -> `pages/app.html#embodied-app`
- `glossary.js` 新增应用层术语：
  - Agentic Workflow
  - Codex
...
- 应用层的护城河不是“接入大模型”，而是工作流嵌入深度、私有数据反馈、权限审计、结果责任和分发入口。

## 2026-05-21 更新：知识卡弹窗内容复核与补强

用户指出：

- 弹窗里的内容不能是空泛模板，必须言之有物。
- 截图示例中“推理模型”知识卡仍是兜底模板，信息密度不够。
...
- 补齐芯片层高频按钮知识卡：
  ...
  - 软件生态
- 改写通用兜底知识卡：
  - 不再出现“当前已接入知识卡框架”这类空话。
  - 即使未覆盖，也会显示产业位置、为什么重要、研究时看什么、后续补强方向。

校验：

- 已确认上述 33 个高频按钮全部有正式知识卡。
- 截图中的“推理模型”已覆盖正式知识卡。
- 脚本语法通过。

## 2026-05-21 更新：横向产业链开始填充 - 大模型训练链

用户要求进入【横向】；先从第一条【大模型训练链 Training Chain】开始。

已完成：

- 新增独立横向产业链页：
  - `pages/training-chain.html`
- 页面覆盖：
  - 当前行业现状
  - 已核验事实
  - 经典拓扑流向
...
- 首页双维矩阵中 5 个“训练链”交点已跳转到训练链页对应位置：
  - 能源层 × 训练链
  - 芯片层 × 训练链
  - 基础设施层 × 训练链
  - 模型层 × 训练链
...
- Meta Engineering - Prometheus and gigawatt-scale AI clusters
- NVIDIA - GB200 NVL72
- OpenAI - The Stargate Project
- xAI - Colossus
- xAI - Colossus 1 compute partnership
- Meta Engineering - AI hardware reliability

关键结论：

- 训练链的核心不再是“买到多少 GPU”，而是有效训练吞吐：电力、HBM、CoWoS、网络、存储、调度、MTBF、数据配比和评测闭环共同决定前沿模型能力。
- 训练链的价值捕获主要集中在 GPU/HBM/先进封装、网络与光互联、AI 数据中心与电力、GPU 云、数据工程和训练软件。

## 2026-05-28 更新：横向产业链继续填充 - AI 推理服务链

用户要求“继续之前的工作”。根据接力记录，上一轮已完成横向第一条【大模型训练链】，本轮继续第二条【AI 推理服务链 Inference Chain】。

已完成：

- 新增独立横向产业链页：
  - `pages/inference-chain.html`
- 页面覆盖：
  - 当前行业现状
  - 已核验事实
  - 经典拓扑流向
...
- 首页双维矩阵中 5 个“推理链”交点已跳转到推理链页对应位置：
  - 能源层 × 推理链
  - 芯片层 × 推理链
  - 基础设施层 × 推理链
  - 模型层 × 推理链
...
- NVIDIA - Dynamo
- NVIDIA Dynamo Docs - High Level Architecture
- NVIDIA Newsroom - Dynamo for reasoning models
- Google - Ironwood TPU for inference
- Google Cloud - Ironwood TPUs and Axion VMs
- OpenAI API Docs - Prompt Caching
- OpenAI - Prompt Caching in the API
- Anthropic Docs - Context windows and thinking tokens
- Anthropic Docs - Extended thinking tips

关键结论：

- 推理链已经成为 AI 商业化的主战场。训练决定模型能力上限，推理决定毛利、用户体验和企业应用能否规模化。
- 推理链的核心瓶颈是 TTFT、P99、reasoning token 爆炸、KV Cache 显存占用、prefill/decode 资源错配、batching 与 SLA 冲突。
- 价值捕获集中在推理芯片、推理运行时、模型路由、推理云、缓存与上下文基础设施、评测和上线治理。

## 2026-05-28 更新：横向产业链继续填充 - 企业 AI 落地链

用户继续要求推进。根据横向顺序，本轮完成第三条【企业 AI 落地链 Enterprise AI Chain】。

已完成：

- 新增独立横向产业链页：
  - `pages/enterprise-chain.html`
- 页面覆盖：
  - 当前行业现状
  - 已核验事实
  - 经典拓扑流向
...
- 首页双维矩阵中 5 个“企业落地链”交点已跳转到企业落地链页对应位置：
  - 能源层 × 企业落地链
  - 芯片层 × 企业落地链
  - 基础设施层 × 企业落地链
  - 模型层 × 企业落地链
  - 应用层 × 企业落地链
- `glossary.js` 新增企业落地链术语：
  - Data Governance
  - Data Lineage
  - Data Residency
  - ETL/ELT
...
- Salesforce - Agentforce 3 visibility and control
- ServiceNow - New AI Platform
- ServiceNow - Yokohama release
- Microsoft - GraphRAG GitHub
- Microsoft GraphRAG Docs
- Databricks - Agent Bricks governed enterprise agent platform

关键结论：

- 企业 AI 的瓶颈不是“能不能调模型”，而是能不能穿过数据治理、权限继承、系统集成、审计合规、责任边界和 ROI 审核。
- 价值捕获集中在系统入口、数据平台、连接器、Agent 编排、行业工作流、安全审计和咨询集成。

## 2026-05-28 更新：横向产业链继续填充 - AI 数据中心建设链

用户继续推进横向链条。本轮完成第四条【AI 数据中心建设链 Data Center Chain】。

已完成：

- 新增独立横向产业链页：
  - `pages/datacenter-chain.html`
- 页面覆盖：
  - 当前行业现状
  - 已核验事实
  - 经典拓扑流向
...
- 首页双维矩阵中 5 个“数据中心链”交点已跳转到数据中心链页对应位置：
  - 能源层 × 数据中心链
  - 芯片层 × 数据中心链
  - 基础设施层 × 数据中心链
  - 模型层 × 数据中心链
  - 应用层 × 数据中心链
- `glossary.js` 新增数据中心链术语：
  - Substation
  - Power Transformer
  - Switchgear
  - Busway
...
- Uptime Intelligence - AI and cooling methods
- Vertiv - NVIDIA GB200 NVL72 power and cooling blueprint
- Schneider Electric - AI reference designs for NVIDIA GB300 NVL72
- Eaton - Grid-to-chip AI data center infrastructure
- NVIDIA - GB200 NVL72 designs to OCP
- NVIDIA - GB200 NVL72

关键结论：

- AI 数据中心建设链的核心不是“机房面积”，而是可交付 MW、并网周期、高密液冷、机柜级系统交付和长期运维可靠性。
- 价值捕获集中在电力接入、土地卡位、高压配电、UPS、液冷、AI 机柜、数据中心开发、数字孪生和运维软件。

## 2026-05-28 更新：横向产业链继续填充 - AI 芯片供应链

用户继续推进横向链条。本轮完成第五条【AI 芯片供应链 Semiconductor Chain】。

已完成：

- 新增独立横向产业链页：
  - `pages/semiconductor-chain.html`
- 页面覆盖：
  - 当前行业现状
  - 已核验事实
  - 经典拓扑流向
...
- 首页双维矩阵中 5 个“芯片供应链”交点已跳转到芯片供应链页对应位置：
  - 能源层 × 芯片供应链
  - 芯片层 × 芯片供应链
  - 基础设施层 × 芯片供应链
  - 模型层 × 芯片供应链
  - 应用层 × 芯片供应链
- `glossary.js` 新增芯片供应链术语：
  - EDA
  - IP
  - PDK
  - Tape-out
...
- TSMC - 2025 Annual Report
- Synopsys - Certified EDA flows for TSMC A16/N2P and CoWoS
- Cadence - TSMC AI flows, 3D-IC, IP and photonics
- Micron - HBM4 for NVIDIA Vera Rubin
- Samsung Semiconductor - HBM4 mass production
- SK hynix - HBM4 development and mass production system
- Applied Materials - Next-gen chipmaking products for AI
- ASML - 2025 Annual Report

关键结论：

- AI 芯片供应链的核心瓶颈不是单点晶体管，而是 EDA/IP、先进制程、先进封装、HBM、基板、测试和系统集成的复合产能。
- 价值捕获集中在 EDA/IP、先进代工与封装、HBM、半导体设备、基板/OSAT/ATE、整机系统集成。

## 2026-05-29 更新：横向产业链继续填充 - 具身智能与机器人链

用户继续推进横向链条。本轮完成第六条【具身智能与机器人链 Embodied AI Chain】。

已完成：

- 新增独立横向产业链页：
  - `pages/embodied-chain.html`
- 页面覆盖：
  - 当前行业现状
  - 已核验事实
  - 经典拓扑流向
...
- 首页双维矩阵中 5 个“机器人链”交点已跳转到具身智能链页对应位置：
  - 能源层 × 机器人链
  - 芯片层 × 机器人链
  - 基础设施层 × 机器人链
  - 模型层 × 机器人链
  - 应用层 × 机器人链
- `glossary.js` 新增具身智能链术语：
  - Sim-to-Real
  - Actuator
  - Harmonic Drive
  - Motion Planning
...
- Figure - Helix VLA model
- Physical Intelligence - π0
- Physical Intelligence - π0.7
- Agility Robotics - Digit and Arc
- Boston Dynamics - Electric Atlas
- Mercedes-Benz - Apollo humanoid robots in production

关键结论：

- 具身智能的核心瓶颈不是“模型是否会聊天”，而是多模态策略能否跨过 Sim-to-Real、端侧低延迟、硬件功耗、本体成本、可靠维护和真实场景数据闭环。
- 价值捕获集中在机器人基础模型、仿真平台、执行器和核心零部件、端侧计算、RaaS 与真实数据飞轮。

## 2026-05-29 更新：横向产业链继续填充 - AI Agent 智能体协同链

用户继续推进横向链条。本轮完成第七条【AI Agent 智能体协同链 Agentic Workflow Chain】。

已完成：

- 新增独立横向产业链页：
  - `pages/agent-chain.html`
- 页面覆盖：
  - 当前行业现状
  - 已核验事实
  - 经典拓扑流向
...
- 首页双维矩阵中 5 个“Agent链”交点已跳转到 Agent 链页对应位置：
  - 能源层 × Agent链
  - 芯片层 × Agent链
  - 基础设施层 × Agent链
  - 模型层 × Agent链
  - 应用层 × Agent链
- `glossary.js` 新增 Agent 链术语：
  - Agent Runtime
  - Planning
  - Memory
  - Sandbox
...
- Agent 链的价值来自“能行动”，核心风险也来自“能行动”。生产级 Agent 必须把 Planning、Memory、Tools/MCP、Sandbox、HITL 和 Audit 组织成可控执行系统。
- 价值捕获集中在 Agent runtime、工具协议与连接器、记忆层、安全权限、评测观测和垂直 Agent 应用。

## 2026-05-29 更新：横向产业链完成首轮填充 - AI 安全与评测链

用户继续推进横向链条。本轮完成第八条【AI 安全与评测链 Safety & Evaluation Chain】。

已完成：

- 新增独立横向产业链页：
  - `pages/safety-chain.html`
- 页面覆盖：
  - 当前行业现状
  - 已核验事实
  - 经典拓扑流向
...
- 首页双维矩阵中 5 个“安全评测链”交点已跳转到安全评测链页对应位置：
  - 能源层 × 安全评测链
  - 芯片层 × 安全评测链
  - 基础设施层 × 安全评测链
  - 模型层 × 安全评测链
  - 应用层 × 安全评测链
- `glossary.js` 新增安全评测链术语：
  - Data Provenance
  - Eval Harness
  - Guardrails
  - Jailbreak
...
- 安全评测链的核心不是“让模型更保守”，而是把数据合规、guardrails、对齐、红队、动态 benchmark、运行时监控和事故响应组成发布门槛。
...
- 新增 `investment-data.js`，把投资研究数据从页面逻辑中拆出，便于后续持续追加。
- Epic 1【系统化公司卡片】：
...
- 新增 `matrix-deepdives.js`，独立承载 5 层 × 8 链的 40 张矩阵交点研究卡。
...
- 新增 `roadmap-data.js`，独立承载 2024-2027E 技术路线图数据。
...
- 新增 `sources-data.js`，建立统一来源库。
- 新增 `pages/sources.html`，建立独立来源库页面。
- 新增 `page-sources.js`，支持来源搜索和筛选。
- 首页导航新增“来源”入口。
- 首页全局搜索已纳入来源库对象。
- 来源库当前包含 25 条来源，覆盖：
...
- 新增 `source-badges.js`，提供可复用来源标签组件。
...
- `investment-data.js` 中 11 个 KPI 全部新增数据化字段：
  - `unit`：单位。
  - `range`：数值/观察区间。
  - `confidence`：置信度。
  - `updated`：更新时间。
...
- 新增 `company-expansion-data.js`，作为公司卡扩展数据层：
  - 保留原有 24 家核心公司。
  - 新增 109 家公司/平台。
  - 当前公司卡总数达到 133 家。
- 所有公司卡统一补齐投资研究字段：
...
新增网站能力：
- 新增顶部分类入口【敏感性】。
- 新增独立面板【估值 / CAPEX 敏感性】，不依赖实时股价，聚焦产业变量、CAPEX 兑现和价值捕获方向。
- 新增数据文件 `valuation-sensitivity-data.js`。
- 敏感性主题、变量和公司标签均已接入知识弹窗。
- 敏感性主题与变量已接入全局搜索。
...
## 2026-06-22 更新：P3.3 公司研究页体验优化

本轮从 P3.3【公司研究页体验优化】开始，保持静态站与现有数据结构不变，只优化公司页搜索、筛选和详情阅读路径。

本轮调整：
- 公司筛选按钮增加数量，例如 `全部 224`、`芯片层 94`，降低筛选前的规模判断成本。
- 公司搜索从单字符串匹配升级为多关键词 AND 匹配，支持更精确组合检索。
- 搜索命中的公司名、ticker、别名、护城河、受益逻辑、风险和主题标签增加轻量高亮。
- 公司结果区新增当前条件条，显示当前筛选 / 搜索条件，并支持一键清除。
- 公司摘要卡新增【研究卡】快捷入口，有深度研究卡的公司可直接打开弹窗，不必先展开详情。
- 公司卡保留原有【详情】折叠区，现有受益逻辑、风险、主题、产品、来源和深度研究内容未删除。

验证：
- `script.js`、`glossary.js`、`page-sources.js`、`matrix-deepdives.js` JS 语法校验通过。
- 本地浏览器验证公司页默认状态：224 家公司池，默认展示 24 张摘要卡，筛选数量正确显示。
- 搜索 `HBM` 命中 23 家公司，搜索高亮与当前条件条正常显示。
- 点击 NVIDIA 卡片【研究卡】可打开深度研究弹窗，包含产业位置、价值驱动、关键 KPI、接口摩擦、催化剂、同业对照和来源锚点。
- 点击 `芯片层` 筛选后显示 94 家匹配，清除筛选可恢复 `全部 224`。

## 2026-06-22 更新：项目核心目标校准

用户明确项目主要用途不是纯技术站，也不是单纯的专业投研终端，而是：
...
新的核心定位：
- `AI 产业链学习笔记 + 科普展示页 + 股票研究资料库`。
- 后续内容应少写抽象战略话术，多写“这是什么、为什么重要、产业链位置、相关公司、炒股时看什么、风险是什么、来源是什么”。

本轮调整：
- README 项目定位从“投资研究 / 创业战略 / 研究终端”改为“个人学习 / 朋友科普 / 股票研究”。
- README 增加三层用途说明：自己学习、分享科普、辅助股票研究。
- 首页 hero 文案同步为“学习 / 科普 / 股票研究”定位。
- 首页首屏新增三张用途卡：自己学习、朋友科普、股票研究。
- 首页公司研究区文案从“投资研究卡 / 投研工具”调整为“公司学习 / 股票研究工具”。
- 页脚从“研究终端版”调整为“学习 / 科普 / 股票研究版”。
- 保留免责声明：公司与产业判断不构成投资建议、证券推荐或交易指令。

验证：
- README 与首页已无对外可见的 `Investment Terminal`、`Research Terminal`、`研究终端`、`投资研究`、`创业战略` 旧定位表述。

## 2026-06-22 更新：首页阅读路径导览

本轮继续围绕“个人学习、朋友科普、股票研究”目标，补齐首页新用户导览。

本轮调整：
- 在首页总览区新增【第一次打开，从这三条路读】导览。
- 三条阅读路径分别对应：
  - 快速入门 AI 产业链：从五层框架开始。
  - 给朋友讲清楚：从双维矩阵和层间联系开始。
  - 研究相关股票：进入公司研究页。
- 每条路径都用“读什么 / 为什么读 / 去哪里”的结构，降低第一次打开网站的理解成本。
- 新增 `.reader-path-grid` 与 `.reader-path-card` 样式，桌面三列、移动端单列，不改变原有导航结构。

验证：
- `script.js`、`glossary.js`、`page-sources.js`、`matrix-deepdives.js`、`source-badges.js` JS 语法校验通过。
- 全站 15 个 HTML 文件本地引用检查通过，缺失为 0。
- 本地浏览器验证：首页导览标题和三张路径卡正常渲染。
- 点击【进入公司研究】可切换到公司研究页，并显示 24 张默认公司摘要卡。

## 2026-06-22 更新：公司股票研究检查清单

本轮继续补齐“辅助股票研究”用途，让用户进入公司研究页后先看到判断顺序，而不是直接进入公司列表。

本轮调整：
- 在公司研究页工具入口下方新增【公司股票研究检查清单】。
- 检查清单分为 6 步：
  - 产业位置：判断公司在五层和八链中的位置。
  - 受益逻辑：判断它受益于电力、GPU、HBM、光模块、数据中心、推理、Agent 或机器人中的哪个变量。
  - 护城河：区分技术、产能、客户、生态、成本、认证周期与短期题材。
  - 验证指标：用订单、毛利、产能、PUE、HBM 带宽、CoWoS、TTFT、P99 等指标跟踪兑现。
  - 同业比较：放到同一价值节点里比较，避免只看单家公司叙事。
  - 风险与来源：核验估值、周期、客户集中、技术替代和来源质量，并保留“不构成投资建议”提醒。
- 新增 `.stock-research-guide` 样式，桌面三列、移动端单列。

验证：
- `script.js`、`glossary.js`、`page-sources.js`、`matrix-deepdives.js`、`source-badges.js` JS 语法校验通过。
- 全站 15 个 HTML 文件本地引用检查通过，缺失为 0。
- 本地浏览器验证：公司研究页显示 6 个检查项，包含“不构成投资建议”提示。
- 公司页默认仍显示 224 家公司池和 24 张摘要卡，未破坏原有公司列表。

## 2026-06-22 更新：首页朋友科普解释链

本轮继续补齐“分享科普”用途，让朋友第一次打开网站时能直接理解“AI 不只是大模型”。

本轮调整：
- 在首页总览区新增【AI 不只是大模型，而是一条从电到应用的链】。
- 用五步解释 AI 产业链：
  - 电力：能源、机房容量、土地、并网、冷却和备用电源。
  - 芯片：GPU、ASIC、HBM、先进封装和光模块。
  - 基础设施：数据中心、云平台、网络、存储、液冷和调度系统。
  - 模型：基础模型、多模态模型、推理模型和行业模型。
  - 应用：Agent、办公、编程、医疗、金融、工业和机器人。
- 新增 `.simple-chain` 样式，桌面五列、移动端单列。

验证：
- `script.js`、`glossary.js`、`page-sources.js`、`matrix-deepdives.js`、`source-badges.js` JS 语法校验通过。
- 全站 15 个 HTML 文件本地引用检查通过，缺失为 0。
- 本地浏览器验证：首页总览页正常显示五步解释链，包含电力、芯片、基础设施、模型、应用五张卡。
- 未发现工作记录验证占位文字出现在页面中。

## 2026-06-22 更新：首页学习高频词速查

本轮继续补齐“自己学习”用途，让初学者先从反复出现的核心概念建立直觉。

本轮调整：
- 在首页总览区新增【先懂这些高频词，再看公司和产业链】。
- 高频词入口覆盖：
  - PUE：数据中心能源效率。
  - HBM：显存与带宽瓶颈。
  - CoWoS：先进封装与高端 AI 芯片供给。
  - GPU 利用率：算力效率。
  - KV Cache：推理成本与长上下文显存压力。
  - MCP：Agent 连接工具和业务系统的协议入口。
- 每个入口复用现有知识弹窗机制，不新增独立词典页。
- 新增 `.starter-term-grid` 样式，桌面三列、移动端单列。

验证：
- `script.js`、`glossary.js`、`page-sources.js`、`matrix-deepdives.js`、`source-badges.js` JS 语法校验通过。
- 全站 15 个 HTML 文件本地引用检查通过，缺失为 0。
- 本地浏览器验证：首页显示 6 个高频词速查卡，均带知识弹窗 payload。
- 点击 `PUE` 速查卡可打开知识弹窗，标题、说明和 4 个内容段落正常显示。

## 2026-06-22 更新：阶段 1 首页定位与阅读路径收口

用户确认先完成阶段 1：首页定位与阅读路径收口。本轮不新增新页面，不改公司、来源、KPI、深度页和搜索数据，只收紧首页结构。

本轮调整：
- 首页收成三段式结构：
  - 首屏定位：说明这是 `AI 产业链学习笔记 + 科普展示页 + 股票研究资料库`，保留五层蛋糕视觉。
  - 学习路线：把原先“第一次打开三条路”和“AI 不只是大模型”合并为一个三步学习路线。
  - 高频概念入口：保留 `PUE / HBM / CoWoS / GPU 利用率 / KV Cache / MCP` 六个速查卡。
- 删除首页重复表达：
  - 移除首屏三用途卡。
  - 移除独立“三条阅读路径”卡组。
  - 移除独立“五步科普链”卡组。
  - 移除总览区“核心逻辑”重复说明。
- 主导航保留：总览、五层、产业链、公司研究、搜索、来源。
- 公司股票研究检查清单继续保留在公司研究页，不放首页。
- 清理不再使用的 `.hero-purpose`、`.reader-path-*`、`.simple-chain` 样式，新增统一 `.learning-route-*` 样式。

验证：
- `script.js`、`glossary.js`、`page-sources.js`、`matrix-deepdives.js`、`source-badges.js` JS 语法校验通过。
- 全站 15 个 HTML 文件本地引用检查通过，缺失为 0。
- 本地浏览器验证：首页首屏显示三用途定位，学习路线显示 3 张路线卡，高频概念入口显示 6 张速查卡。
- 点击 `PUE` 速查卡可打开知识弹窗，标题、说明和内容段落正常显示。
- 主导航 `五层 / 产业链 / 公司研究 / 搜索` 可切换到对应面板，`来源` 可进入来源库页面。
- 390px 移动端视口验证：学习路线和高频概念入口均为单列，无横向溢出。

## 2026-06-23 更新：阶段 2 公司研究页第二轮体验优化

用户确认执行阶段 2：公司研究页第二轮体验优化。本轮不扩公司数据，不新增重点赛道页，不改来源、KPI、深度页和搜索索引结构，只基于现有字段派生公司研究视图。

本轮调整：
- 在公司搜索栏下方新增紧凑研究工具条：
  - 排序：默认排序、AI 暴露度优先、风险从高到低、已深挖优先、A/港股/中概优先。
  - 研究状态筛选：全部、已深挖、有来源、来源待加强、A/H 股。
  - 重点关注分组：高 AI 暴露度、已深挖、A/H 股、风险较高。
- 公司列表渲染顺序调整为：层级/链条筛选 → 搜索 → 研究状态筛选 → 重点关注分组 → 排序 → 分页展示。
- 当前条件条增加状态、重点和排序条件，并支持单项清除。
- 公司卡新增轻量研究状态标签：
  - 已深挖：有 deepDive。
  - 有来源：有 sourceIds。
  - 来源待加强：sourceIds 较少。
  - A/H 股：ticker 为 `.SZ / .SS / .HK`。
- 重点关注分组不做本地收藏或个人观察池，只作为静态学习和股票研究入口。

验证：
- `script.js`、`glossary.js`、`page-sources.js`、`matrix-deepdives.js`、`source-badges.js` JS 语法校验通过。
- 全站 15 个 HTML 文件本地引用检查通过，缺失为 0。
- 本地浏览器验证：公司研究页默认仍为 224 家公司池，默认展示 24 张摘要卡。
- 搜索 `HBM` + 筛选 `芯片层` + 排序 `AI 暴露度优先` 可组合使用，结果为 23 家，首位 AI 暴露度为极高。
- `已深挖 / 有来源 / 来源待加强 / A/H 股` 状态筛选可改变结果数量，并正确显示 active 状态。
- 点击重点关注【高 AI 暴露度】后，结果和当前条件条同步更新。
- 展开 NVIDIA【详情】可直接阅读深度研究内容，不再需要额外研究卡弹窗。
- 同业对照、量化 KPI、估值敏感性入口可正常切换。
- 390px 移动端视口验证：公司工具条和公司卡无横向溢出。

## 2026-06-23 更新：公司研究页移除重复研究卡弹窗入口

用户反馈公司研究页里的“研究卡”弹窗与详情里的深度研究内容重复，直接在页面展开阅读即可。

本轮调整：
- 移除公司摘要卡上的【研究卡】快捷按钮。
- 移除公司详情深度研究区里的【展开研究卡】按钮。
- 保留公司详情内的深度研究内容，包括价值驱动、关键 KPI、接口摩擦、催化剂和同业对照。
- 保留投资主题、代表产品等小知识弹窗；本轮只移除公司深度研究的重复弹窗入口。

验证：
- `script.js` JS 语法校验通过。
- 本地浏览器验证：搜索 `NVIDIA` 后，公司卡不再出现【研究卡】或【展开研究卡】按钮。
- 展开 NVIDIA【详情】后，可直接看到深度研究内容，且深度研究区内不再包含额外知识弹窗按钮。
